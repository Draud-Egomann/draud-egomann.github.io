...
<template>
  <div :class="height" class="w-full px-8 relative bg-heroBanner">
    <div
      class="h-full flex flex-col justify-center items-center md:items-baseline gap-8 container mx-auto"
    >
      <div class="w-full flex flex-col gap-4 py-4">
        <h1
          class="font-extrabold text-white text-center lg:text-left text-3xl lg:text-4xl my-2 animate-fade-right"
        >
          {{ title }}
        </h1>

        <h2
          class="font-bold text-white text-center lg:text-left text-2xl lg:text-3xl my-2 animate-fade-right"
        >
          {{ subTitle }}
        </h2>
        ...
        >
          {{ buttonText }}
          <i class="pi pi-arrow-right !ml-4 animate-bounce-horizontal"></i>
        </Button>
      </div>
      ...
      <fa-icon
        ...
        :aria-label="$t('components.scrollDown')"
      />
    </div>
  </div>
</template>
